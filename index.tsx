...
const loadState = () => {
    ...
    if (storedFreelancing) {
        state.freelancingHistory = JSON.parse(storedFreelancing).map((entry: any) => ({ 
            ...entry, 
    ...
    if (storedSelling) {
        state.sellingHistory = JSON.parse(storedSelling).map((entry: any) => ({ 
            ...entry, 
            id: entry.id || Date.now().toString() + Math.random().toString(36).substring(2),
    ...
    const loadOptions = (key: string, defaultOptions: string[]) => {
...
const renderConfirmationModal = (title: string, message: string, onConfirm: () => void, onCancel: () => void) => {
...
const renderOptionManagementModal = (title: string, options: string[], optionsKey: string, onClose: () => void, onAdd: (newValue: string, key: string) => void, onEdit: (originalValue: string, newValue: string, key: string) => void, onDelete: (valueToDelete: string, key: string) => void) => {
    const modalDiv = document.createElement('div');
    modalDiv.className = 'modal-overlay';
    modalDiv.setAttribute('role', 'dialog');
    modalDiv.setAttribute('aria-modal', 'true');
    ...
    const renderOptionList = (currentOptions: string[]) => `
    ...
    const newOptionInput = modalDiv.querySelector('#new-option-input') as HTMLInputElement;
    const addNewOptionBtn = modalDiv.querySelector('#add-new-option-btn');
    if (addNewOptionBtn && newOptionInput) {
        // Fix: Explicitly cast newOptionInput to HTMLInputElement to access .value
        addNewOptionBtn.addEventListener('click', () => {
            const newValue = (newOptionInput as HTMLInputElement).value.trim();
            if (newValue && !options.includes(newValue)) {
                onAdd(newValue, optionsKey);
                (newOptionInput as HTMLInputElement).value = ''; // Clear input after adding
    ...
    modalDiv.addEventListener('click', (e) => {
        const target = e.target as HTMLElement; // Fix: Cast e.target to HTMLElement
        // Fix: Explicitly cast target to HTMLElement to access classList and dataset
        if (target.classList.contains('edit-option-btn')) {
            const valueToEdit = (target as HTMLElement).dataset.value;
            if (valueToEdit) {
            ...
            const valueToDelete = (target as HTMLElement).dataset.value;
            ...
            const originalValue = (target as HTMLElement).dataset.originalValue;
            // Fix: Explicitly cast target to HTMLElement to access closest
            const inputElement = (target as HTMLElement).closest('.option-item')?.querySelector('.option-edit-input') as HTMLInputElement;
...
const renderHistoryScreen = (title: string, history: any[], onClose: () => void, onEditEntry: (id: string, newAmount: number, type: string) => void, onDeleteEntry: (id: string, type: string) => void, showTypeColumn: boolean, tabType: string | null) => {
    ...
    historyScreenDiv.querySelectorAll('tbody tr').forEach(rowElement => {
        // Fix: Explicitly cast rowElement to HTMLElement to access dataset
        ...
        const handleLongPressStart = (event: MouseEvent | TouchEvent) => {
        ...
        const handleLongPressEnd = (event: MouseEvent | TouchEvent) => {
            if (state.longPressTimer) {
                clearTimeout(state.longPressTimer);
        ...
        const addEndListeners = (element: HTMLElement) => {
        ...
        if (state.editingId === entry.id) {
            // Fix: Explicitly cast input to HTMLInputElement
            const input = row.querySelector('.history-edit-input') as HTMLInputElement;
            if (input) {
                input.addEventListener('input', (e) => { state.editingAmount = (e.target as HTMLInputElement).value; });
                input.focus();
            }
...
const renderTabMenuOverlay = (type: string, menuTitle: string, menuItems: {className: string, ariaLabel: string, text: string, onClick: () => void}[]) => {
    ...
    // Fix: Convert boolean to string for aria-hidden attribute
    ...
    const handleClickOutside = (event: MouseEvent | TouchEvent) => {
        if (!menuDiv.contains(event.target as Node) && (!menuFab || !menuFab.contains(event.target as Node))) {
            if (type === 'freelancing') state.showFreelancingMenu = false;
            else state.showSellingMenu = false;
            renderApp();
...
const renderApp = () => {
    ...
    if (state.showOverallHistory) {
        const combinedHistory = [
            ...state.freelancingHistory.map((entry: any) => ({ ...entry, type: 'Freelancing' })),
            ...state.sellingHistory.map((entry: any) => ({ ...entry, type: 'Selling' }))
    ...
    if (state.showFreelancingHistory) {
        renderHistoryScreen(
            "Freelancing",
            state.freelancingHistory.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    ...
    if (state.showSellingHistory) {
        renderHistoryScreen(
            "Selling",
            state.sellingHistory.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    ...
    if (state.showOptionManagementModal && state.editingOptionsFor) {
        ...
        // Type assertions for optionsKey to access state correctly
    ...
    const freelancingIncome = state.freelancingHistory.reduce((sum: number, entry: any) => sum + entry.amount, 0);
    const sellingIncome = state.sellingHistory.reduce((sum: number, entry: any) => sum + entry.amount, 0);
    ...
    if (state.activeTab === 'home') {
        ...
        setTimeout(() => { 
            const incomeInput = document.getElementById('freelancing-income-input') as HTMLInputElement;
            const dateInput = document.getElementById('freelancing-date-input') as HTMLInputElement;
            const sourceSelect = document.getElementById('freelancing-source-select') as HTMLSelectElement;
            const typeSelect = document.getElementById('freelancing-type-select') as HTMLSelectElement;
            const noteInput = document.getElementById('freelancing-note-input') as HTMLTextAreaElement;
            const saveButton = document.getElementById('save-freelancing-entry-btn');

            // Fix: Explicitly cast e.target to the correct HTML element type
            if (incomeInput) incomeInput.addEventListener('input', (e) => { state.freelancingInput = (e.target as HTMLInputElement).value; });
            if (dateInput) dateInput.addEventListener('change', (e) => { state.freelancingDateInput = (e.target as HTMLInputElement).value; });
            if (sourceSelect) sourceSelect.addEventListener('change', (e) => { state.freelancingSourceInput = (e.target as HTMLSelectElement).value; });
            if (typeSelect) typeSelect.addEventListener('change', (e) => { state.freelancingTypeInput = (e.target as HTMLSelectElement).value; });
            if (noteInput) noteInput.addEventListener('input', (e) => { state.freelancingNoteInput = (e.target as HTMLTextAreaElement).value; });
        ...
        setTimeout(() => { // Ensure elements are in DOM
            const incomeInput = document.getElementById('selling-income-input') as HTMLInputElement;
            const dateInput = document.getElementById('selling-date-input') as HTMLInputElement;
            const sourceSelect = document.getElementById('selling-source-select') as HTMLSelectElement;
            const typeSelect = document.getElementById('selling-type-select') as HTMLSelectElement;
            const noteInput = document.getElementById('selling-note-input') as HTMLTextAreaElement;
            const saveButton = document.getElementById('save-selling-entry-btn');

            // Fix: Explicitly cast e.target to the correct HTML element type
            if (incomeInput) incomeInput.addEventListener('input', (e) => { state.sellingInput = (e.target as HTMLInputElement).value; });
            if (dateInput) dateInput.addEventListener('change', (e) => { state.sellingDateInput = (e.target as HTMLInputElement).value; });
            if (sourceSelect) sourceSelect.addEventListener('change', (e) => { state.sellingSourceInput = (e.target as HTMLSelectElement).value; });
            if (typeSelect) typeSelect.addEventListener('change', (e) => { state.sellingTypeInput = (e.target as HTMLSelectElement).value; });
            if (noteInput) noteInput.addEventListener('input', (e) => { state.sellingNoteInput = (e.target as HTMLTextAreaElement).value; });
            if (saveButton) saveButton.addEventListener('click', () => {
    ...
    appContainer.querySelectorAll('.tab-button').forEach(button => {
        button.addEventListener('click', (e) => {
            // Fix: Explicitly cast e.currentTarget to HTMLButtonElement to access dataset
            if (state.activeTab === (e.currentTarget as HTMLButtonElement).dataset.tab) return; // Don't re-render if clicking active tab

            state.activeTab = (e.currentTarget as HTMLButtonElement).dataset.tab as string;
...
const handleEditHistoryEntry = (id: string, newAmount: number, type: string) => {
...
const handleDeleteHistoryEntry = (id: string, type: string) => {
    if (type === 'freelancing' || type === 'Freelancing') {
        state.freelancingHistory = state.freelancingHistory.filter((entry: any) => entry.id !== id);
    } else if (type === 'selling' || type === 'Selling') {
        state.sellingHistory = state.sellingHistory.filter((entry: any) => entry.id !== id);
    }
    saveState();
    // renderApp() is explicitly called after confirmation in renderHistoryScreen
};

const handleAddOption = (newValue: string, optionsKey: string) => {
    let optionsArray = (state as any)[optionsKey]; // Use 'any' for dynamic key access
    if (!optionsArray.includes(newValue)) {
        optionsArray.push(newValue);
        (state as any)[optionsKey] = [...optionsArray].sort(); // Keep sorted
        saveState();
    }
};

const handleEditOption = (originalValue: string, newValue: string, optionsKey: string) => {
    let optionsArray = (state as any)[optionsKey]; // Use 'any' for dynamic key access
    if (originalValue !== newValue && !optionsArray.filter((opt: string) => opt !== originalValue).includes(newValue)) {
...
const handleDeleteOption = (valueToDelete: string, optionsKey: string) => {
    let optionsArray = (state as any)[optionsKey]; // Use 'any' for dynamic key access
    if (optionsArray.length > 1) { // Prevent deleting last option
        (state as any)[optionsKey] = optionsArray.filter((opt: string) => opt !== valueToDelete);
        saveState();
        // Adjust default input if deleted option was selected
        if (optionsKey === 'freelancingSourceOptions' && state.freelancingSourceInput === valueToDelete) state.freelancingSourceInput = (state as any)[optionsKey][0] || '';
        if (optionsKey === 'freelancingTypeOptions' && state.freelancingTypeInput === valueToDelete) state.freelancingTypeInput = (state as any)[optionsKey][0] || '';
        if (optionsKey === 'sellingSourceOptions' && state.sellingSourceInput === valueToDelete) state.sellingSourceInput = (state as any)[optionsKey][0] || '';
        if (optionsKey === 'sellingTypeOptions' && state.sellingTypeInput === valueToDelete) state.sellingTypeInput = (state as any)[optionsKey][0] || '';
    } else {

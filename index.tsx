...
const renderOptionManagementModal = (title: string, options: string[], optionsKey: string, onClose: () => void, onAdd: (newValue: string, optionsKey: string) => void, onEdit: (originalValue: string, newValue: string, optionsKey: string) => void, onDelete: (valueToDelete: string, optionsKey: string) => void) => {
    const modalDiv = document.createElement('div');
    modalDiv.className = 'modal-overlay';
    modalDiv.setAttribute('role', 'dialog');
    modalDiv.setAttribute('aria-modal', 'true');
    ...
    const newOptionInput = modalDiv.querySelector<HTMLInputElement>('#new-option-input');
    const addNewOptionBtn = modalDiv.querySelector('#add-new-option-btn');
    if (addNewOptionBtn && newOptionInput) {
        // Fix: Added type assertion for newOptionInput.value
    ...
    modalDiv.addEventListener('click', (e) => {
        ...
        // Fix: Added type assertion for target.classList and target.dataset
        ...
        // Fix: Added type assertion for target.classList and target.dataset
            ...
            // Fix: Added type assertion for target.dataset and target.closest
            const originalValue = target.dataset.originalValue;
            const inputElement = target.closest('.option-item')?.querySelector('.option-edit-input') as HTMLInputElement | null; // Fix: Cast querySelector result
        ...
        // Fix: Added type assertion for target.classList
...
const renderHistoryScreen = (title: string, history: any[], onClose: () => void, onEditEntry: (id: string, newAmount: number, type: string) => void, onDeleteEntry: (id: string, type: string) => void, showTypeColumn: boolean, tabType: string | null) => {
        ...
        historyTableHtml = `
            <div class="table-container">
                <table class="history-table">
                    <thead>
                        <tr>
                            <th class="history-table-header">S. No.</th>
                            <th class="history-table-header">Date</th>
                            ${showTypeColumn ? '<th class="history-table-header">Category</th>' : ''}
                            <th class="history-table-header">Source</th>
                            <th class="history-table-header">Type</th>
                            <th class="history-table-header amount-header">Amount</th>
                            <th class="history-table-header">Note</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${history.map((entry, index) => `
                            ...
                            >
                                ...
                                <td class="amount-cell" style="position: relative;">
                                    ...
                                    ` : `
                                        <span class="amount-display amount-value-text ${state.activeRowForActions === entry.id ? 'hidden' : ''}">${formatCurrency(entry.amount)}</span>
    ...
    historyScreenDiv.querySelectorAll('tbody tr').forEach(rowElement => {
        ...
        if (state.editingId === entry?.id) {
            const input = row.querySelector('.history-edit-input') as HTMLInputElement | null;
            if (input) {
                // Fix: Added type assertion for e.target.value
            ...
            if (editBtn) editBtn.addEventListener('click', (e) => {
                ...
                if (entry) { // Ensure entry is not null before accessing its amount
...
const renderApp = () => {
    ...
    if (state.activeTab === 'home') {
        ...
        setTimeout(() => { 
            ...
            // Fix: Added type assertions for input, select, and textarea elements
            if (incomeInput) (incomeInput as HTMLInputElement).addEventListener('input', (e) => { state.freelancingInput = (e.target as HTMLInputElement).value; });
            if (dateInput) (dateInput as HTMLInputElement).addEventListener('change', (e) => { state.freelancingDateInput = (e.target as HTMLInputElement).value; });
            if (sourceSelect) (sourceSelect as HTMLSelectElement).addEventListener('change', (e) => { state.freelancingSourceInput = (e.target as HTMLSelectElement).value; });
            if (typeSelect) (typeSelect as HTMLSelectElement).addEventListener('change', (e) => { state.freelancingTypeInput = (e.target as HTMLSelectElement).value; });
            if (noteInput) (noteInput as HTMLTextAreaElement).addEventListener('input', (e) => { state.freelancingNoteInput = (e.target as HTMLTextAreaElement).value; });
        ...
        setTimeout(() => { // Ensure elements are in DOM
            ...
            // Fix: Added type assertions for input, select, and textarea elements
            if (incomeInput) (incomeInput as HTMLInputElement).addEventListener('input', (e) => { state.sellingInput = (e.target as HTMLInputElement).value; });
            if (dateInput) (dateInput as HTMLInputElement).addEventListener('change', (e) => { state.sellingDateInput = (e.target as HTMLInputElement).value; });
            if (sourceSelect) (sourceSelect as HTMLSelectElement).addEventListener('change', (e) => { state.sellingSourceInput = (e.target as HTMLSelectElement).value; });
            if (typeSelect) (typeSelect as HTMLSelectElement).addEventListener('change', (e) => { state.sellingTypeInput = (e.target as HTMLSelectElement).value; });
            if (noteInput) (noteInput as HTMLTextAreaElement).addEventListener('input', (e) => { state.sellingNoteInput = (e.target as HTMLTextAreaElement).value; });
            if (saveButton) saveButton.addEventListener('click', () => {
    ...
    appContainer.querySelectorAll('.tab-button').forEach(button => {
        button.addEventListener('click', (e) => {
            // Fix: Added type assertion for e.currentTarget.dataset
            if (state.activeTab === (e.currentTarget as HTMLElement).dataset.tab) return; // Don't re-render if clicking active tab

            // Fix: Added type assertion for e.currentTarget.dataset
            state.activeTab = (e.currentTarget as HTMLElement).dataset.tab as string;
...
const handleAddIncome = (type: 'freelancing' | 'selling') => {
...
const handleAddOption = (newValue: string, optionsKey: keyof typeof state) => {
...
const handleEditOption = (originalValue: string, newValue: string, optionsKey: keyof typeof state) => {
    let optionsArray = state[optionsKey] as string[];
...
export {}; // Fix: Convert to a module to prevent global scope conflicts
